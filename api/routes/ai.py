...
from api.routes.ai_data_info import (
...
def ai_status():
    ...
    if os.path.exists(uploads_folder):
        csv_files = [
            f for f in os.listdir(uploads_folder) if f.lower().endswith(".csv")
        ]
        csv_file_count += len(csv_files)
        if csv_files:
    ...
    return jsonify(
        {
            ...
            "database_path": current_app.config.get(
                "DATABASE_PATH", "medical_billing.db"
            ),
...
def chat_with_ai():
    """Universal conversational AI that can answer any question about dataset."""
    try:
        ...
        if "compare" in message.lower() and len(provider_names) == 2:
            try:
                current_app.logger.info(
                    f"Routing to provider comparison: {provider_names}"
                )
                from api.routes.ai_functions import compare_providers_enhanced

                comparison_result = compare_providers_enhanced(
                    provider_names[0], provider_names[1]
                )
                ...
                return jsonify(
                    {
                        "response": (
                            "I encountered an error performing the comparison: "
                            f"{str(e)}"
                        )
                    }
                )
        ...
        if provider_names and provider_overhead_terms:
            try:
                ...
                current_app.logger.info(
                    f"Routing to {target_provider} overhead coverage analysis"
                )

                # Get the provider's overhead analysis data
                import requests

                base_url = "http://localhost:5001/api/analytics"
                analysis_url = (
                    f"{base_url}/provider-overhead-analysis/{target_provider}"
                )
...
Please analyze this data and provide a conversational, business-focused 
response about whether {target_provider} is covering the overhead expenses. 
Use ONLY the numbers provided above. Do not make up any figures.
"""

                        response = call_ollama_with_enhanced_context(
                            enhanced_prompt, conversation_context, memory
                        )
                        ...
                        return jsonify(
                            {
                                "response": (
                                    f"I encountered an error analyzing "
                                    f"{target_provider}'s overhead coverage: "
                                    f"{error_msg}"
                                )
                            }
                        )
                    ...
                    return jsonify(
                        {
                            "response": (
                                f"I couldn't retrieve the overhead analysis for "
                                f"{target_provider}. The analysis service may "
                                f"be unavailable."
                            )
                        }
                    )
                ...
                current_app.logger.error(
                    f"Error in {target_provider} overhead analysis: {e}"
                )
                return jsonify(
                    {
                        "response": (
                            f"I encountered an error analyzing "
                            f"{target_provider}'s overhead coverage: {str(e)}"
                        )
                    }
                )
...
Respond as Ada, their intelligent medical billing assistant. Be 
conversational, insightful, and helpful.
...
Response:"""
        ...
        response = call_ollama_with_enhanced_context(
            user_prompt, conversation_context, memory
        )
        ...
        if response and len(response) > 20:
            memory.store_memory(
                ...
                context={
                    ...
                    "response_type": "universal_conversational",
                },
                importance=7,
        ...
        error_response = (
            ...
            "like provider performance or revenue trends."
        )
        return (
            jsonify(
                {
...
def get_data_info():
    ...
    try:
        ...
        return jsonify({"files_info": files_info, "database_info": db_info})
    except Exception as e:
...
def call_ollama(prompt, system_message=None):
    ...
    if system_message:
        ...
        default_system = (
            "You are Ada, a helpful and conversational database AI assistant. "
            "Respond in a natural, friendly tone. Offer meaningful insights "
            "about the data and suggest next steps when appropriate. Be "
            "concise but thorough, and don't be afraid to ask clarifying "
...
def make_conversational(response, question, history):
    """Make AI response more conversational using Ada's memory and
    personality system."""
    try:
...
Response:"""
    ...
    is_first_message = (
        len(history) <= 2
    )  # Only user's first message and possibly the welcome
    ...
    if "provider" in response.lower() and "revenue" in response.lower():
        ...
        follow_ups = [
            "Would you like me to help you analyze any specific data from "
            "these files?",
            "What specific insights are you looking for from your billing " "data?",
            "Would you like to see revenue trends or provider performance "
            "from this data?",
        ]
        response = f"{response}\n\n{random.choice(follow_ups)}"
        ...
        follow_ups = [
            ...
            "Would you like to see which providers are most affected by "
            "these issues?",
    ...
    if is_first_message and not any(
        x in response.lower() for x in ["follow_ups", "would you like"]
    ):
        tips = [
            "\n\nYou can ask me about specific providers, revenue analysis, "
            "payer comparisons, or data quality issues. What would you like "
            "to explore?",
            "\n\nIs there a specific aspect of your billing data you'd like "
            "to analyze?",
...
def extract_provider_names(message):
    ...
    try:
        ...
        all_providers = sorted(
            [p["provider_name"] for p in all_providers], key=len, reverse=True
        )
        provider_names = []
...
def extract_user_name(history):
    ...
    for message in history:
        if message.get("role") == "user":
            ...
            name_match = (
                re.search(r"my name is (\w+)", content)
                or re.search(r"i am (\w+)", content)
                or re.search(r"i'm (\w+)", content)
            )
            if name_match:
...
def get_top_provider():
    ...
    try:
        conn = get_db_connection()
        query = """
        SELECT p.provider_name, SUM(pt.cash_applied) as total_revenue, 
        ...
        if result and result["total_revenue"]:
            ...
            return (
                f"The top performing provider is {provider} with a total "
                f"revenue of ${revenue:,.2f} from {count} transactions."
            )
        else:
            return (
                "I couldn't find any provider revenue data in the database. "
                "Please check that you have payment transactions with valid "
                "provider IDs."
            )
    except Exception as e:
        current_app.logger.error(f"Error getting top provider: {e}")
        return (
            "I encountered an error while trying to retrieve provider data. "
...
def get_all_providers():
    ...
    try:
        ...
        cursor.execute(
            ...
            WHERE p.active = 1 
                AND p.provider_name NOT IN 
                    ('Test Provider', 'Another Provider', 'Unknown')
        ...
        if not providers:
            return (
                "I don't see any providers in the database yet. You may need "
                "to upload some CSV files with provider data first."
            )
        ...
        response += (
            "\nWould you like more details about any specific provider or "
            "their performance?"
        )
        return response
        ...
        return (
            "I encountered an error while retrieving the provider list. "
...
def get_provider_details(provider_name):
    ...
    Returns:
        str: Formatted provider details including revenue, transactions, and
             recent activity
    """
    try:
        conn = get_db_connection()
        ...
        if not result:
            conn.close()
            available_providers = get_all_providers_list()
            providers_str = ", ".join([p["provider_name"] for p in available_providers])
            return (
                f"I couldn't find a provider named '{provider_name}' in the "
                f"database. Here are the available providers: {providers_str}"
            )
        ...
        if result["first_transaction"] and result["last_transaction"]:
            first_date = result["first_transaction"]
            last_date = result["last_transaction"]
            response += f"• Active Period: {first_date} to {last_date}\n"
        ...
        if monthly_data:
            ...
            for month_data in monthly_data:
                ...
                response += (
                    f"• {month}: {transactions} transactions, " f"${revenue:,.2f}\n"
                )
        ...
        if top_payers:
            ...
            for payer in top_payers:
                ...
                payer_revenue = payer["revenue"]
                payer_transactions = payer["transactions"]
                response += (
                    f"• {payer_name}: ${payer_revenue:,.2f} "
                    f"({payer_transactions} transactions)\n"
                )

        provider_name = result["provider_name"]
        response += (
            f"\nWould you like more specific analysis or comparisons for "
            f"{provider_name}?"
        )
        return response
        ...
        current_app.logger.error(
            f"Error getting provider details for {provider_name}: {e}"
        )
        return (
            f"I encountered an error while retrieving details for "
            f"{provider_name}. Please check the database connection."
        )
...
def get_all_providers_list():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT provider_name FROM providers WHERE active = 1 "
...
def format_business_analysis(analysis_result):
    """Formats the structured reasoning result into a natural language
    response."""
    if not analysis_result or "error" in analysis_result:
        return "I couldn't analyze the business question based on the provided " "data."

    question = analysis_result.get("question", "Unknown question")
    response = f"🧠 **Business Analysis for:** {question}\n\n"

    # Show question classification
    if analysis_result.get("question_type"):
        q_type = analysis_result["question_type"].title()
        response += f"**Analysis Type:** {q_type}\n\n"
    ...
    response += (
        "Would you like me to dive deeper into any specific aspect or run "
        "additional analysis?"
    )
    return response
...
You are analyzing medical billing data for a healthcare practice. Be 
knowledgeable about:
...
- Vary your language and tone to avoid repetition"""
        ...
        if response:
            ...
            return (
                ...
                "rephrase your question or try asking about something specific "
                "like provider performance or revenue trends?"
            )
    ...
    try:
        ...
        if provider_names:
            if len(provider_names) == 2:
                # Provider comparison
                try:
                    comparison_data = compare_providers_enhanced(
                        provider_names[0], provider_names[1]
                    )
                    p1, p2 = provider_names[0], provider_names[1]
                    data_context += f"\n=== PROVIDER COMPARISON: {p1} vs {p2} ===\n"
        ...
        if any(
            term in message_lower
            for term in ["payer", "insurance", "bcbs", "aetna", "payment", "claims"]
        ):
        ...
        return (
            "I encountered an issue accessing your data. Please try your "
            "question again."
        )


def extract_provider_names_universal(message, cursor):
    """Extract provider names from message using universal provider
    detection"""
    try:
...
def get_provider_summary(provider_name, cursor):
    ...
    try:
        ...
        if summary:
            return (
                f"{summary[0]} transactions, ${summary[1]:,.2f} revenue, "
                f"${summary[2]:.2f} avg payment, {summary[5]} payers, "
                f"active {summary[3]} to {summary[4]}"
            )
...
def get_business_summary(cursor):
    ...
    try:
        ...
        if summary:
            return (
                f"Last 12 months: {summary[0]:,} transactions, "
                f"${summary[1]:,.2f} revenue, {summary[2]} active providers, "
                f"period {summary[3]} to {summary[4]}"
            )
...
def get_payer_summary(cursor):
    ...
    try:
        ...
        if payers:
            ...
            for payer in payers:
                result += (
                    f"- {payer[0]}: {payer[1]} transactions, " f"${payer[2]:,.2f}\n"
                )
            return result
...
def get_period_summary(period, cursor):
    ...
    try:
        # This is a simplified version - you can enhance based on the
        # period format
        cursor.execute(
            """
            SELECT 
        ...
        if summary:
            return (
                f"Recent period: {summary[0]} transactions, "
                f"${summary[1]:,.2f} revenue, {summary[2]} providers active"
            )
...
def get_general_summary(cursor):
    ...
    try:
        ...
        if summary:
            return (
                f"Database overview: {summary[0]:,} total transactions, "
                f"${summary[1]:,.2f} total revenue, {summary[2]} providers, "
                f"{summary[3]} payers"
            )
...
def get_expense_summary(cursor):
    ...
    try:
        ...
        for expense in expenses:
            ...
            result += (
                f"  - {expense[1]}: ${expense[2]:,.2f} ({expense[4]} "
                f"payments, {expense[6]} frequency)\n"
            )
        ...
        cursor.execute(
            ...
            SELECT 
                ...
                COUNT(CASE WHEN amount > budgeted_amount THEN 1 END) 
                    as over_budget_count,
                COUNT(CASE WHEN amount < budgeted_amount THEN 1 END) 
                    as under_budget_count
            FROM expense_transactions
        ...
        if variance_data and variance_data[0] is not None:
            ...
            result += (
                f"Items over budget: {variance_data[1]}, "
                f"under budget: {variance_data[2]}\n"
            )
...
def call_ollama_optimized(prompt, system_message, config):
    ...
    ollama_url = config.get("homelab_url") or get_config().get(
        "ollama.laptop_url", "http://localhost:11434"
    )
        ...
        response = requests.post(
            ...
            json={
                ...
                "options": {
                    ...
                    "repeat_penalty": config.get("options", {}).get(
                        "repeat_penalty", 1.1
                    ),
                    "presence_penalty": config.get("options", {}).get(
                        "presence_penalty", 0.1
                    ),
                    "frequency_penalty": config.get("options", {}).get(
...
def call_ollama_with_enhanced_context(user_prompt, conversation_context, memory):
    """Enhanced Ollama call with optimized configuration and comprehensive
    medical billing context."""
    try:
        # Import the optimization manager with error handling
        try:
                ...
                for line in lines:
                    if line.startswith("User: "):
                        conversation_history.append(
                            {"role": "user", "content": line[6:]}
                        )
                    elif line.startswith("Ada: "):
                        conversation_history.append(
                            {"role": "assistant", "content": line[5:]}
                        )

            system_prompt = optimizer.get_enhanced_system_prompt(
                question_context=user_prompt, conversation_history=conversation_history
            )

            # Try optimized call first
            ...
            current_app.logger.warning(
                f"Optimization system failed, falling back to regular mode: "
                f"{optimization_error}"
            )

        # Fallback to regular system prompt and call
        conversation_history = []
        if conversation_context:
            # Parse conversation context into history format
            lines = conversation_context.strip().split("\n")
        ...
        # Use basic system prompt for fallback
        system_prompt = f"""You are Ada, an intelligent medical billing AI 
assistant with deep expertise in healthcare revenue analysis.
...
IMPORTANT: Use ONLY the data provided in the user's message. Do not invent 
or hallucinate any financial figures."""

        # Use regular Ollama call as fallback
        ...
        if response:
            ...
            return (
                "I'm having trouble processing that right now. Could you "
                "rephrase your question? I can help you analyze provider "
                "performance, revenue trends, payer relationships, or any "
                "other aspect of your medical billing data using your actual "
                "data."
            )

    except Exception as e:
        current_app.logger.error(f"Error in enhanced Ollama call: {e}")
        # Final fallback to regular call_ollama
        try:
            basic_prompt = (
                f"You are Ada, a medical billing AI assistant. Help analyze "
                f"the user's question: {user_prompt}"
            )
            return call_ollama(user_prompt, basic_prompt)
