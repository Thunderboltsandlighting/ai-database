...
def update_personality():
    ...
    try:
        ...
        if 'communication_style' in data:
            if personality_manager.set_communication_style(data['communication_style']):
                updated.append(f"Communication style: {data['communication_style']}")
            else:
                errors.append(f"Invalid communication style: {data['communication_style']}")
        
        if 'analysis_depth' in data:
            if personality_manager.set_analysis_depth(data['analysis_depth']):
                updated.append(f"Analysis depth: {data['analysis_depth']}")
            else:
                errors.append(f"Invalid analysis depth: {data['analysis_depth']}")
        
        if 'proactive_suggestions' in data:
            personality_manager.enable_proactive_suggestions(data['proactive_suggestions'])
            updated.append(f"Proactive suggestions: {'enabled' if data['proactive_suggestions'] else 'disabled'}")
...
def apply_personality_preset(preset_name):
    ...
    try:
        ...
        if 'communication_style' in settings:
            personality_manager.set_communication_style(settings['communication_style'])
            updated.append(f"Communication style: {settings['communication_style']}")
        
        if 'analysis_depth' in settings:
            personality_manager.set_analysis_depth(settings['analysis_depth'])
            updated.append(f"Analysis depth: {settings['analysis_depth']}")
        
        if 'proactive_suggestions' in settings:
            personality_manager.enable_proactive_suggestions(settings['proactive_suggestions'])
            updated.append(f"Proactive suggestions: {'enabled' if settings['proactive_suggestions'] else 'disabled'}")
...
def reset_ada_config():
    ...
    try:
        ...
        return jsonify({
            'message': 'Ada configuration reset to defaults',
            'status': 'success'
        })
