<template>
  <v-container fluid>
    ...
    <v-row class="mb-6">
        ...
        <v-card color="primary" dark>
          <v-card-text>
            <div class="d-flex align-center">
              ...
              <div>
                <div class="text-h5">${{ formatCurrency(metrics.totalRevenue) }}</div>
        ...
        <v-card color="secondary" dark>
          <v-card-text>
            <div class="d-flex align-center">
              ...
              <div>
                <div class="text-h5">{{ metrics.activeProviders }}</div>
                <div class="text-body-2">Active Providers</div>
                <div class="text-caption mt-1">{{ metrics.totalTransactions }} transactions</div>
              </div>
            </div>
          </v-card-text>
        </v-card>
        ...
        <v-card color="success" dark>
          <v-card-text>
            <div class="d-flex align-center">
              ...
              <div>
                <div class="text-h5">${{ formatCurrency(metrics.monthlyAverage) }}</div>
                <div class="text-body-2">Monthly Average</div>
                <div class="text-caption mt-1">Last 6 months</div>
              </div>
            </div>
          </v-card-text>
        </v-card>
      ...
      <v-col cols="12" sm="6" md="3">
        <v-card :color="overheadCoverageColor" dark>
          <v-card-text>
            <div class="d-flex align-center">
              <v-icon size="40" class="mr-3">mdi-shield-check</v-icon>
              <div>
                <div class="text-h5">{{ metrics.overheadCoverage }}%</div>
                <div class="text-body-2">Overhead Coverage</div>
                <div class="text-caption mt-1">${{ formatCurrency(metrics.monthlyOverhead) }} needed</div>
              </div>
            </div>
          </v-card-text>
        </v-card>
        ...
        <v-card class="mb-4">
          ...
          <v-card-text>
            <div v-if="aiInsights.length === 0" class="text-center py-4">
              ...
              <div class="text-body-2 mt-2">Generating insights...</div>
            </div>
            <div v-else>
              <v-alert
        ...
        <v-card>
          ...
          <v-card-text>
            <div class="text-center">
              ...
              <div class="mt-3">
                <div class="text-body-1 font-weight-medium">{{ dataQuality.status }}</div>
                <div class="text-body-2 text-grey">{{ dataQuality.issues.length }} issues found</div>
              </div>
            </div>
            <v-divider class="my-3"></v-divider>
            <div v-if="dataQuality.issues.length > 0">
              <div class="text-body-2 font-weight-medium mb-2">Recent Issues:</div>
              <v-chip
    ...
    const loadDashboardData = async () => {
      ...
      try {
        ...
        const [
          ...
          databaseApi.getSummary(),
          analysisApi.getProviderSummary(),
          analysisApi.getAnalytics()
        ])

        // Process summary data
        if (summaryResponse.data.success) {
          const summary = summaryResponse.data.summary
          metrics.value.totalRevenue = summary.total_revenue || 0
          metrics.value.activeProviders = summary.active_providers || 0
          metrics.value.totalTransactions = summary.total_transactions || 0
          metrics.value.monthlyAverage = summary.monthly_average || 0
        }

        // Process provider data
        if (providersResponse.data.success) {
          providerPerformance.value = providersResponse.data.providers.map(provider => ({
            ...provider,
            monthly_contribution: (provider.total_revenue * (provider.company_percentage / 100)) / 12,
            coverage_percentage: ((provider.total_revenue * (provider.company_percentage / 100)) / 12) / metrics.value.monthlyOverhead * 100
          }))
        }

        // Process analytics data
        if (analyticsResponse.data.success) {
          revenueChartData.value = analyticsResponse.data.revenue_trends
          providerChartData.value = analyticsResponse.data.provider_comparison
        }
