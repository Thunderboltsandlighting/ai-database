...
def test_tables_created(db):
    ...
    """
    # Use the same database connection as the fixture
...
def test_insert_provider(db):
    """Test provider insertion functionality using SQLAlchemy ORM"""
    # Create a new provider using ORM
    session = db.get_session()

    # Check that Dr. Smith already exists (from fixture)
    existing_provider = (
        session.query(Provider).filter(Provider.provider_name == "Dr. Smith").first()
    )
    assert existing_provider is not None, "Dr. Smith should exist from fixture"

    # Test inserting a new provider
    new_provider = Provider(
        provider_name="Dr. Test", specialty="Cardiology", npi_number="1234567890"
    )
    session.add(new_provider)
    session.commit()

    assert new_provider.provider_id > 0, "Provider ID should be a positive integer"

    # Test retrieving the provider
    retrieved_provider = (
        session.query(Provider)
        .filter(Provider.provider_id == new_provider.provider_id)
        .first()
    )
    assert retrieved_provider is not None, "Provider should exist in database"
    assert retrieved_provider.provider_name == "Dr. Test", "Provider name should match"
    assert (
        retrieved_provider.specialty == "Cardiology"
    ), "Provider specialty should match"
    assert retrieved_provider.npi_number == "1234567890", "Provider NPI should match"
...
def test_get_provider_revenue(db):
    ...
    assert all(
        provider_only_df["provider_name"] == "Dr. Revenue"
    ), "All rows should be for Dr. Revenue"
...
def test_data_quality_functions(db):
    ...
    assert all(
        pd.isna(missing_df["cash_applied"])
    ), "All rows should have NULL cash_applied"
    ...
    cursor = db.conn.execute(
        "SELECT COUNT(*) FROM data_quality_issues WHERE issue_type = 'missing_value'"
    )
...
def test_total_revenue_2024(db):
    ...
    assert (
        alpha_row.iloc[0]["total_2024_revenue"] == 1100.0
    ), "Dr. Alpha's revenue should be 1100.0"
    ...
    assert (
        beta_row.iloc[0]["total_2024_revenue"] == 1100.0
    ), "Dr. Beta's revenue should be 1100.0"
